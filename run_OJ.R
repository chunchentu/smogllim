...
train_diff = train_t - gllim_pred_t[, train_index, drop=FALSE]
train_mse = mean(apply(train_diff^2, 2, sum))
test_diff = test_t - gllim_pred_t[, test_index, drop=FALSE]
test_mse = mean(apply(test_diff^2, 2, sum))
cat(sprintf("Train MSE: %.4f, test MSE: %.4f\n", train_mse, test_mse))
...
smogllim(train_t, train_y, K, M, in_r=in_r2, Lw=Lw, cstr=cstr,
